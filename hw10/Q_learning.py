...
if __name__ == "__main__":
    ...
    for i in range(EPISODES):
        ...
        while (not done):
            ...
            # Q(s,a) = (1 - α)Q(s,a) + α(r + γmaxQ(s', a')) if !done
            # Q(s,a) = 1-LEARNING_RATE * Q_table + LEARNING_RATE(reward + DISCOUNT_FACTOR*maxQ)
            # Q(s,a) = (1 - α)Q(s,a) + αr if done
            # s = state, a = action, r = reward, α = learning rate, γ = discount factor

            prediction = np.array([Q_table[(n_obs, i)] for i in range(env.action_space.n)])
            
            new_action = np.argmax(prediction)

            maxQ = prediction[new_action]

            Q_table[(obs,action)] = ((1 - LEARNING_RATE) * Q_table[(obs,action)]) + LEARNING_RATE * (reward + DISCOUNT_FACTOR * maxQ)
